...
function renderPlayers() {
  ...
  for (const player of state.players) {
    ...
    if (topDiscard && topDiscard.type !== "action" && !state.gameOver && !state.pendingAction && state.turnPhase === "draw" && player.id !== state.activePlayer) {
      discardTopEl.classList.add("clickable");
      discardTopEl.title = `Draw top of ${player.name}'s discard`;
      discardTopEl.addEventListener("click", () => drawFromDiscard(player.id));
    }
    ...
    player.hand.forEach((card, idx) => {
      ...
      if (isActive && !state.gameOver && !state.pendingAction) {
        ...
        el.addEventListener("drop", (e) => {
          ...
          const arr = state.players[state.activePlayer].hand;
    ...
    if (isActive) {
      ...
      if (state.turnPhase === "discard") {
        ...
        if (canMeld) {
          ...
          meldBtn.addEventListener("click", meldSelected);
        ...
        if (selCount === 1 && getRemainingDiscards() > 0) {
          ...
          discardBtn.addEventListener("click", () => discardCardFromHand(state.selectedIndices[0]));
